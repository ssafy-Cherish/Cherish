...
import { dailyFetch } from "../../services/diaryService";
...
const DiaryDailyPage = () => {
  ...
  const { data: dailyData } = useQuery({
    ...
    queryFn: () => dailyFetch(coupleId, dayjs(date).format("YYYY-MM-DD")),
    staleTime: 60000,
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-3 w-full gap-6">
            ...
            <div className="me-auto relative col-span-2 w-full">
              <ul className="timeline timeline-snap-icon max-md:timeline-compact timeline-vertical  overflow-y-auto h-[35vw] rmscroll">
                {dailyData &&
                  dailyData.meeting.map((meeting, mIdx) => (
            ...
            <div className="flex justify-end items-center">
              ...
              >
                <div className="max-w-full h-full grid grid-rows-6 justify-center absolute">
                  ...
                  >
                    {dailyData?.memo ? dailyData.memo.content : "작성된 메모가 없습니다."}
                  </div>
                </div>
      ...
      <AnimatePresence>
        ...
        {openMemoModal && <MemoModal onClose={closeMemoModal} memo={dailyData.memo}></MemoModal>}
      </AnimatePresence>
    </>
  );
};
